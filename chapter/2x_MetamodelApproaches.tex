...
\begin{itemize}
	...
	\item \emph{EClass} models classes. A class refers a number of other classes as its \code{supertypes}. They are identified by name and contain a number of attributes and references which both are structural features.
...
\end{xtxt}
In this example, it's important to note that \code{MyEvent} is the referenced types \code{EClass}, not a parser rule. The \kode{|VALID_EVENT_ID} is optional and refers to an \code{EDataType} rule or uses a string by default.
