...
\section{Implement Methodology, Steps} 
EMF because of tool reuse (editor, persistence, validation, transformation model to model etc.)

Summarize Steps
Research: GMF allows multiple visual representations by Notation model and determines initial type by Extensible Type Registry. <<< desirable
Research: Textual Projectional Editor MPS \& Evaluation MPS: Text != Text. Text not free form but template structured. Usability issues. 1:1 relation view and abstract data structure.
EMF Textual: Unstable, re-eval.
Barista : not available. Projectional editor with parser based on \emph{incremental Parser} of Harmonia for \emph{stable structures}.
Harmonia: \emph{Stable Structures} AND \emph{Sentential Forms}
Proxima: \emph{structured token} for presentation, complex layered architecture, vis edit on other level. bi di m2m trans,

Evaluate XText: 
Problems
\begin{itemize}
	\item full model described, 
	\item unstable, 
	\item no alternative text rep
	\item structure bound
\end{itemize}
Chances
\begin{itemize}
	\item PTC decides presentation
	\item node model
\end{itemize}

Idea:
make textual presentations switchable by guiding the PTC.
needed: distinguish productions. -> created notation model ... complex as parse tree!
result: use notation model as parse tree.
=> Notation Metamodel + gPTC = switchable textual notation.

Structured Tokens for parser -> fits EObject. How to char stream to integrate with Xtext ser? 
ID -> Map. ID Char. UTF8 enc. private use char. emf lexer.
=> EObj on char stream

Sentential Forms => Combine Notation Model w. EMFLexer + post process grammar

Struc Token as Presentation => omit PTC for token. Hint from Not MM is enough.

Discuss impact unstable models.
