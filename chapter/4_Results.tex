...
The Xtext framework was examined afterwards in respect of adding alternative representations and related problems. The following central perceptions were made:
\begin{itemize}
...
To enable multiple representations, the alternatives need to be made available. This is done by the notation model, which is described in \ref{chp:NotMM}. The notation model is able to be equivalent to a parse tree, but also allows to refer to following production parts, called hints. EMFs \code{ECrossReferenceAdapter} allows navigation of unidirectional references in both direections. This allows the use or reuse of notation model elements which refer to AST nodes just by getting all referring \code{EObject}s and filtering a type.
